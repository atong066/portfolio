    ...
    <motion.main className="min-h-screen" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: reduceMotion ? 0 : 0.6, ease }}>
      <motion.section ref={heroRef} id="top" style={{ opacity: heroOpacity }} className="relative grid min-h-screen grid-rows-[auto_1fr] overflow-clip px-[.22rem] pt-[.22rem] pb-[.3rem] isolate max-[1080px]:min-h-auto max-[1080px]:pb-[.42rem] max-[720px]:px-[.14rem] max-[720px]:pt-[.16rem] max-[720px]:pb-[.18rem]">
        ...
        <motion.header initial={{ opacity: 0, y: reduceMotion ? 0 : -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: reduceMotion ? 0 : 0.7, ease }} className="relative z-[2] flex items-center justify-between gap-[.15rem] max-[720px]:flex-col max-[720px]:items-start max-[720px]:gap-[.08rem]">
          <motion.a className="text-[.15rem] font-extrabold tracking-[.34em] text-inherit no-underline max-[720px]:w-full max-[720px]:text-[.38rem] max-[720px]:tracking-[.18em] max-[720px]:text-left" href="#top" whileHover={reduceMotion ? undefined : { y: -2 }}>VIRGILIO</motion.a>
        ...
        <motion.div style={{ y: heroY }} initial="hidden" animate="visible" variants={heroVariants(reduceMotion)} className="relative z-[1] max-w-[6.8rem] self-end pt-[1.05rem] pb-[.36rem] max-[1080px]:pt-[.72rem] max-[1080px]:pb-0 max-[720px]:max-w-full max-[720px]:pt-[.24rem] max-[720px]:text-left">
          <motion.p variants={itemVariants(reduceMotion, 0)} className="mb-[.12rem] text-[.13rem] uppercase tracking-[.24em] text-[var(--text-muted)] max-[720px]:mb-[.12rem] max-[720px]:text-[.15rem] max-[720px]:tracking-[.18em]">Full-stack web applications</motion.p>
          <motion.h1 variants={itemVariants(reduceMotion, 0.08)} className="m-0 max-w-[6.4rem] font-[var(--font-display)] text-[.9rem] leading-[.94] font-semibold tracking-[-.03em] text-balance max-[720px]:max-w-[4.5rem] max-[720px]:text-[.64rem] max-[720px]:leading-[.92]">
      ...
      <Section id="about" label="Approach">
        <div className="max-w-[11rem]">
          <motion.div initial={{ opacity: 0, y: reduceMotion ? 0 : 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={once} transition={{ duration: reduceMotion ? 0 : 0.7, ease }} className="max-[720px]:relative max-[720px]:overflow-hidden max-[720px]:rounded-[.28rem] max-[720px]:border max-[720px]:border-[rgba(255,221,185,0.12)] max-[720px]:bg-[radial-gradient(circle_at_top_left,rgba(255,196,124,0.12),transparent_36%),linear-gradient(180deg,rgba(255,234,210,0.08),rgba(255,255,255,0.02))] max-[720px]:px-[.18rem] max-[720px]:py-[.2rem] max-[720px]:shadow-[0_.16rem_.38rem_rgba(0,0,0,0.16)]">
            ...
            <p className="hidden max-[720px]:mb-[.08rem] max-[720px]:block max-[720px]:text-[.14rem] max-[720px]:uppercase max-[720px]:tracking-[.2em] max-[720px]:text-[rgba(245,241,232,0.46)]">Design stance</p>
            <motion.p className="m-0 max-w-[6.2rem] text-[.26rem] leading-[1.45] max-[720px]:max-w-[3.45rem] max-[720px]:text-[.26rem] max-[720px]:leading-[1.4]">
          ...
          <div className="mt-[.24rem] grid gap-[.12rem] max-[720px]:mt-[.16rem] max-[720px]:gap-[.1rem]">
            {principles.map((principle, index) => (
              <motion.div key={principle} initial={{ opacity: 0, y: reduceMotion ? 0 : 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={once} transition={{ duration: reduceMotion ? 0 : 0.6, delay: reduceMotion ? 0 : 0.08 * index, ease }} className="border-t border-[rgba(255,255,255,0.08)] pt-[.12rem] max-[720px]:grid max-[720px]:grid-cols-[.32rem_minmax(0,1fr)] max-[720px]:items-start max-[720px]:gap-[.12rem] max-[720px]:rounded-[.18rem] max-[720px]:border max-[720px]:border-[rgba(255,255,255,0.08)] max-[720px]:bg-[radial-gradient(circle_at_left_center,rgba(255,196,124,0.08),transparent_28%),linear-gradient(180deg,rgba(255,255,255,0.03),rgba(255,255,255,0.01))] max-[720px]:px-[.16rem] max-[720px]:py-[.15rem] max-[720px]:shadow-[0_.1rem_.24rem_rgba(0,0,0,0.14)]">
                <span className="hidden h-[.28rem] w-[.28rem] items-center justify-center rounded-full border border-[rgba(255,196,124,0.2)] bg-[rgba(255,196,124,0.08)] text-[.13rem] font-semibold text-[rgba(255,214,169,0.88)] max-[720px]:flex">
                  {index + 1}
                </span>
                <p className="m-0 text-[.16rem] leading-[1.7] text-[rgba(245,241,232,0.72)] max-[720px]:text-[.21rem] max-[720px]:leading-[1.56]">{principle}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <Section id="work" label="Selected work">
        <div className="grid max-w-[11rem] max-[720px]:gap-[.12rem]">
          {projects.map((project, index) => (
            <motion.article key={project.name} initial={{ opacity: 0, y: reduceMotion ? 0 : 26 }} whileInView={{ opacity: 1, y: 0 }} viewport={once} transition={{ duration: reduceMotion ? 0 : 0.7, delay: reduceMotion ? 0 : 0.09 * index, ease }} whileHover={reduceMotion ? undefined : { x: 4 }} className={`grid grid-cols-[1rem_minmax(0,1.5fr)_minmax(2.1rem,1fr)] items-start gap-[.18rem] border-t border-[rgba(255,255,255,0.1)] py-[.22rem] max-[1080px]:grid-cols-1 max-[720px]:gap-[.14rem] max-[720px]:rounded-[.24rem] max-[720px]:border max-[720px]:border-[rgba(255,255,255,0.08)] max-[720px]:bg-[radial-gradient(circle_at_top_right,rgba(111,197,255,0.07),transparent_30%),linear-gradient(180deg,rgba(255,255,255,0.03),rgba(255,255,255,0.015))] max-[720px]:px-[.18rem] max-[720px]:py-[.18rem] max-[720px]:shadow-[0_.12rem_.3rem_rgba(0,0,0,0.16)] ${index === projects.length - 1 ? 'border-b border-b-[rgba(255,255,255,0.1)]' : ''}`}>
              <div className="grid gap-[.04rem] text-[.12rem] uppercase tracking-[.18em] text-[rgba(245,241,232,0.44)] max-[720px]:gap-[.04rem] max-[720px]:text-[.16rem]"><span>{project.id}</span><span>{project.type}</span></div>
              <div>
                <h2 className="mb-[.06rem] font-[var(--font-display)] text-[.42rem] leading-[1] font-semibold tracking-[-.03em] max-[720px]:text-[.36rem]">
                ...
                <p className={`${copyClass} max-[720px]:text-[.16rem] max-[720px]:leading-[1.6]`}>{project.summary}</p>
              </div>
              <p className={`${copyClass} max-w-[2.3rem] max-[1080px]:max-w-none max-[720px]:text-[.16rem] max-[720px]:leading-[1.6]`}>{project.impact}</p>
            </motion.article>
          ))}
        </div>
      </Section>
      ...
      <Section label="Services">
        <div className="grid max-w-[11rem] grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)] gap-[.24rem] max-[1080px]:grid-cols-1">
          <motion.div initial={{ opacity: 0, y: reduceMotion ? 0 : 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={once} transition={{ duration: reduceMotion ? 0 : 0.7, ease }} className="max-[720px]:relative max-[720px]:overflow-hidden max-[720px]:rounded-[.26rem] max-[720px]:border max-[720px]:border-[rgba(255,221,185,0.12)] max-[720px]:bg-[radial-gradient(circle_at_top_left,rgba(255,196,124,0.12),transparent_38%),linear-gradient(180deg,rgba(255,236,212,0.07),rgba(255,255,255,0.02))] max-[720px]:px-[.2rem] max-[720px]:py-[.22rem] max-[720px]:shadow-[0_.16rem_.38rem_rgba(0,0,0,0.18)]">
            ...
            <p className="hidden text-[.16rem] uppercase tracking-[.18em] text-[rgba(245,241,232,0.5)] max-[720px]:mb-[.08rem] max-[720px]:block">Build, refine, ship</p>
            <h2 className="m-0 font-[var(--font-display)] text-[.58rem] leading-[.94] font-semibold tracking-[-.03em] text-balance max-[720px]:max-w-[3.3rem] max-[720px]:text-[.5rem] max-[720px]:leading-[.92]">From visual concept to production-ready frontend.</h2>
            <p className="hidden max-[720px]:mt-[.12rem] max-[720px]:block max-[720px]:max-w-[3.5rem] max-[720px]:text-[.21rem] max-[720px]:leading-[1.55] max-[720px]:text-[rgba(245,241,232,0.72)]">
              I turn interface ideas into web applications that feel intentional on the surface and dependable underneath.
            </p>
          </motion.div>
          <div className="grid content-start gap-[.12rem] max-[720px]:gap-[.1rem]">
            {services.map((item, index) => (
              <motion.article key={item.id} initial={{ opacity: 0, x: reduceMotion ? 0 : 18 }} whileInView={{ opacity: 1, x: 0 }} viewport={once} transition={{ duration: reduceMotion ? 0 : 0.6, delay: reduceMotion ? 0 : 0.08 * index, ease }} className="border-t border-[rgba(255,255,255,0.08)] pt-[.12rem] max-[720px]:grid max-[720px]:grid-cols-[.52rem_minmax(0,1fr)] max-[720px]:items-start max-[720px]:gap-[.12rem] max-[720px]:rounded-[.2rem] max-[720px]:border max-[720px]:border-[rgba(255,255,255,0.08)] max-[720px]:bg-[radial-gradient(circle_at_top_right,rgba(111,197,255,0.06),transparent_32%),linear-gradient(180deg,rgba(255,255,255,0.028),rgba(255,255,255,0.012))] max-[720px]:px-[.16rem] max-[720px]:py-[.16rem] max-[720px]:shadow-[0_.12rem_.28rem_rgba(0,0,0,0.14)]">
                <div className="hidden h-[.42rem] w-[.42rem] items-center justify-center rounded-full border border-[rgba(255,196,124,0.22)] bg-[rgba(255,196,124,0.08)] text-[.16rem] font-semibold tracking-[.08em] text-[rgba(255,214,169,0.88)] max-[720px]:flex">
                  {item.id}
                </div>
                <div>
                  <p className="hidden max-[720px]:mb-[.04rem] max-[720px]:block max-[720px]:text-[.16rem] max-[720px]:uppercase max-[720px]:tracking-[.18em] max-[720px]:text-[rgba(245,241,232,0.45)]">
                    {item.title}
                  </p>
                  <p className="m-0 text-[.16rem] leading-[1.7] text-[rgba(245,241,232,0.72)] max-[720px]:text-[.22rem] max-[720px]:leading-[1.5]">
                    {item.detail}
                  </p>
                </div>
              </motion.article>
      ...
      <Section id="contact" label="Contact">
        <motion.div initial={{ opacity: 0, y: reduceMotion ? 0 : 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={once} transition={{ duration: reduceMotion ? 0 : 0.75, ease }} className="relative max-w-[11rem] pt-[.22rem] max-[720px]:overflow-hidden max-[720px]:rounded-[.28rem] max-[720px]:border max-[720px]:border-[rgba(255,221,185,0.12)] max-[720px]:bg-[radial-gradient(circle_at_top_left,rgba(255,196,124,0.12),transparent_34%),linear-gradient(180deg,rgba(255,236,212,0.06),rgba(255,255,255,0.01))] max-[720px]:px-[.18rem] max-[720px]:py-[.2rem] max-[720px]:shadow-[0_.18rem_.42rem_rgba(0,0,0,0.16)]">
          ...
          <motion.div initial={{ opacity: 0, y: reduceMotion ? 0 : 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={once} transition={{ duration: reduceMotion ? 0 : 0.6, delay: reduceMotion ? 0 : 0.14, ease }} className="max-[720px]:mt-[.2rem] max-[720px]:grid max-[720px]:gap-[.12rem] max-[720px]:rounded-[.2rem] max-[720px]:border max-[720px]:border-[rgba(255,255,255,0.08)] max-[720px]:bg-[radial-gradient(circle_at_top_right,rgba(111,197,255,0.06),transparent_34%),rgba(10,14,25,0.36)] max-[720px]:px-[.14rem] max-[720px]:py-[.14rem]">
            <div className="hidden max-[720px]:block">
              <p className="m-0 text-[.14rem] uppercase tracking-[.18em] text-[rgba(245,241,232,0.42)]">Email</p>
              <p className="mt-[.04rem] text-[.18rem] font-medium text-[rgba(245,241,232,0.8)]">Available for new work</p>
            </div>
            <motion.a href="mailto:galiciavigie0@gmail.com" whileHover={reduceMotion ? undefined : { y: -4, scale: 1.03, boxShadow: '0 .18rem .46rem rgba(255,196,124,.28)' }} whileTap={reduceMotion ? undefined : { scale: 0.98 }} className="mt-[.22rem] inline-flex min-h-[.5rem] items-center justify-center rounded-full bg-[linear-gradient(135deg,#f1e3c8_0%,#ffc47c_100%)] px-[.2rem] py-[.14rem] text-[.15rem] font-bold tracking-[.01em] text-[#0d1421] no-underline shadow-[0_.14rem_.4rem_rgba(255,196,124,0.18)] max-[720px]:mt-0 max-[720px]:w-full max-[720px]:px-[.16rem] max-[720px]:py-[.16rem] max-[720px]:text-[.16rem] max-[720px]:leading-[1.2] max-[720px]:break-all">
...
function Section({ children, id, label }: { children: ReactNode; id?: string; label: string }) {
  return <section id={id} className="grid grid-cols-[minmax(1rem,1.7rem)_minmax(0,1fr)] gap-[.24rem] px-[.22rem] py-[.64rem] max-[1080px]:grid-cols-1 max-[1080px]:gap-[.12rem] max-[720px]:gap-[.06rem] max-[720px]:px-[.14rem] max-[720px]:pt-[.18rem] max-[720px]:pb-[.3rem]"><div className={labelClass}>{label}</div>{children}</section>
}
